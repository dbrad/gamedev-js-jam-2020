import { Align, drawText, drawTexture, textHeight, textWidth, drawQuad } from "../core/draw";
...
export class StoreNode extends SceneNode
{
  ...
  public draw( now: number, delta: number ): void
  {
    ...
    drawText( `${ GameState.storeDeck.length }`.padStart( 2, "0" ), this.topLeft.x + 16, this.topLeft.y + this.size.y - 12, { textAlign: Align.Center, colour: 0x88FFFFFF, font: "gb" } );
    ...
    if ( this.showRefreshTooltip || this.showUpgradeTooltip )
    {
      ...
      drawQuad( topLeft.x, topLeft.y, 80, 40 );
      gl.colour( 0xFF0E0803 );
      drawQuad( topLeft.x + 1, topLeft.y + 1, 78, 38 );
    ...
    if ( this.tooltipCard )
    {
      ...
      gl.colour( 0XFFEEEEEE );
      drawQuad( topLeft.x, topLeft.y, 102, 60 );
      gl.colour( 0xFF0E0803 );
      drawQuad( topLeft.x + 1, topLeft.y + 1, 100, 58 );
      ...
      if ( this.tooltipCard.levelsText.length > 0 && this.tooltipCard.level < this.tooltipCard.maxLevel )
      {
        ...
        gl.colour( 0XFFEEEEEE );
        drawQuad( topLeft.x, topLeft.y, width, height );
        gl.colour( 0xFF0E0803 );
        drawQuad( topLeft.x + 1, topLeft.y + 1, width - 2, height - 2 );
