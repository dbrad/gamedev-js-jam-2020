...
export class DeckSelectorNode extends SceneNode implements Interactive
{
  ...
  public draw( now: number, delta: number ): void
  {
    ...
    drawQuad( topleft.x, topleft.y, size.x, size.y );
    gl.colour( this.colour );
    drawQuad( topleft.x + 1, topleft.y + 1, size.x - 2, size.y - 2 );
    gl.colour( 0xFFFFFFFF );
    drawQuad( topleft.x + ~~( this.markerSize / 2 ), topleft.y + ~~( this.markerSize / 2 ), this.markerSize, this.markerSize );
    ...
    if ( this.selected )
    {
      ...
      drawQuad( topleft.x + ~~( this.markerSize / 2 ) + 3, topleft.y + ~~( this.markerSize / 2 ) + 3, this.markerSize - 6, this.markerSize - 6 );
