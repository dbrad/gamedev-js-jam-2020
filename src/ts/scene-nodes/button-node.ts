  ...
  public draw( now: number, delta: number ): void
  {
    ...
    if ( !this.pressed )
    {
      if ( !this.hover )
      {
        ...
        drawQuad( this.topLeft.x, this.topLeft.y, this.size.x, this.size.y );
        ...
        drawQuad( this.topLeft.x, this.topLeft.y + this.size.y - 2, this.size.x, 2 );
      } else
      {
        // Neutral
        gl.colour( this.colour );
        drawQuad( this.topLeft.x, this.topLeft.y + 1, this.size.x, this.size.y - 1 );
        ...
        gl.colour( 0xBB000000 );
        drawQuad( this.topLeft.x, this.topLeft.y + this.size.y - 1, this.size.x, 1 );
      }
    } else if ( this.pressed )
    {
      // Presses
      gl.colour( this.colour );
      drawQuad( this.topLeft.x, this.topLeft.y + 2, this.size.x, this.size.y - 2 );
